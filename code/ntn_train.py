...
def run_training():
    ...
    train_data, num_entities, num_relations, entity_indices = dataprocessing.index_data(train_data,
                                                                                        entity_list,
                                                                                        entity_indices,
                                                                                        relation_list)
    ...
    with tf.Graph().as_default():
        ...
        for i in range(1, num_iters):
            print('#iter: {}'.format(i))
            data_batch = dataprocessing.generate_corrupting_batch(batch_size,
                                                                  train_data,
                                                                  num_entities,
                                                                  corrupt_size)
            relation_batches = dataprocessing.split_corrupting_batch(data_batch, num_relations)
