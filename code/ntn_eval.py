...
# maybe in get_predictions()
# TODO: do we need `init = tf.initialize_all_variables()` and `sess.run(init)`
...
def run_evaluation():
    ...
    init_embedding_wordvecs, entity_to_wordvec = ntn_input.load_init_embeds(params.data_path)
    # =========================================================================
    ...
    with tf.Graph().as_default():
        ...
        data_placeholders = [tf.placeholder(tf.float32, shape=(None, 3))
        ...
        inference = ntn.g_function(data_placeholders,
                                   corrupt_placeholder,
                                   init_embedding_wordvecs,
                                   entity_to_wordvec,
                                   num_entities,
                                   num_relations,
                                   slice_size,
        ...
        print('Define predicting algorithm')
        eval_correct = ntn.eval(inference)
        saver = tf.train.Saver()
        
        # load training model
        saver.restore(sess, params.output_path + 'around100/Wordnet110.sess')
        saver.restore(sess, params.output_path + 'Wordnet490.sess')
        # init = tf.initialize_all_variables()
        # sess.run(init)
        print('Begin evaluation')
        evaluate_precision(sess,
                           eval_correct,
                           data_placeholders,
                           label_placeholders,
                           corrupt_placeholder,
...
def evaluate_precision(sess,
    ...
    print('precision of training model: ' + str(precision))
...
def get_thresholds():
    ...
    predictions_list = ntn.g_function(batch_placeholder,
                                      corrupt_placeholder,
                                      init_word_embeds,
                                      entity_to_wordvec,
                                      num_entities,
